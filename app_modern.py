...
def load_model(model_path):
    ...
    # Engine-built profiles are stored as JSON inside the .pth file.
    try:
        with open(model_path, "r", encoding="utf-8") as _f:
            data = _f.read(512)
        if data.lstrip().startswith("{"):
            payload = json.loads(data)
            if payload.get("engine") == "world_vocoder":
                # world-vocoder profile -> usable by the precision engine
                svc = None
    ...
    if not SVC_AVAILABLE:
        print("so-vits-svc-fork is not installed; only WORLD-vocoder "
...
class ModernApp:
    ...
    def generate(self):
        try:
            ...
            engine_ok = False
            if svc is None:
                # Fall back to the WORLD-vocoder engine when the loaded model
                # is a precision profile (and the input is text).
                engine_text = (self.textbox.get("1.0", "end").strip()
                               if not self.audio_path.get() else "")
                engine_ok = False
                if engine_text and current_model_path and os.path.exists(current_model_path):
                    try:
                        mdl_name = os.path.basename(os.path.dirname(current_model_path))
                        result = self.precision_system.generate_precision_vocals(
                            engine_text, mdl_name, output_name=f"{name}_precision")
                        if result.get('status') == 'success':
                            outfile = result['output_path']
                            outwav = outfile
                            engine_ok = True
                            self.progress.update_progress(80, "🎤 Engine generated vocals...",
                                                      suggestion="WORLD-vocoder voice conversion complete")
                        else:
                            print("Engine generation error:", result)
                    except Exception as e:
                        print("Engine fallback error:", e)
                if not engine_ok:
